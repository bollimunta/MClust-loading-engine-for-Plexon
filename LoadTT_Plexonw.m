...
% Events
trial_count_indx = find(STR == 11002); % strobe value
STR(trial_count_indx + 1) = STR(trial_count_indx + 1) + 10000;

nStart = sum(STR == 1001); Start = EVts(STR == 1001); % start Strobe
nStop  = sum(STR == 1009); Stop =  EVts(STR == 1009); % stop strobe
if Start(1) > Stop(1)
    Start(1)=[];
    nStart = nStart - 1;
end
